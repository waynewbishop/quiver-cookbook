...
#Playground("The Normal Distribution") {
    ...
    for x in xs {
        let density = Distributions.normal.pdf(x: x, mean: mean, standardDeviation: std) ?? 0
    ...
    let p975 = Distributions.normal.cdf(x: 1.96, mean: mean, standardDeviation: std) ?? 0
    let p50  = Distributions.normal.cdf(x: 0.0,  mean: mean, standardDeviation: std) ?? 0
    ...
    let z975 = Distributions.normal.quantile(p: 0.975, mean: mean, standardDeviation: std) ?? 0
    print("z at p = 0.975 → \(String(format: "%.4f", z975))")  // ≈ 1.9600

    // Out-of-domain input returns nil rather than crashing
    let bad = Distributions.normal.pdf(x: 0, mean: 0, standardDeviation: -1)
    print("standardDeviation = -1 → \(bad as Any)")  // nil
}
